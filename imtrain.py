"""
차량 소리 분류 모델 - 클래스 균형 버전

주요 특징:
- 모든 데이터를 학습에 사용 (검증 데이터도 학습에 포함)
- 클래스 간 동일한 샘플 수로 균형 조정 (중앙값 기준)
- FSD50K, ESC-50에서 최대한 많이 로드
- 부족한 클래스는 있는 그대로 사용 (무리한 증강 없음)
- 멜 스펙트로그램 기반 CNN 분류기
"""
...
class Config:
    ...
    모든 하이퍼파라미터, 경로, 오디오 처리 파라미터를 중앙에서 관리
    """
    ...
    MODEL_NAME = 'vehicle_audio_balanced'    # 저장할 모델 파일명 접두사
...
class DatasetMapper:
    """
    외부 데이터셋(ESC-50, FSD50K)의 클래스를 UrbanSound8K의 10개 클래스로 매핑

    명확하게 대응 가능한 라벨만 선택하여 매핑하며,
    애매한 카테고리는 제외하여 데이터 품질을 유지
    """
...
class BalancedAudioDataset(Dataset):
    ...
    특징:
    - 학습 모드: 모든 데이터셋(UrbanSound8K 전체 + ESC-50 + FSD50K)을 로드하고 클래스 균형 조정
    ...
    def _balance_classes(self):
        ...
        for label in range(10):
            ...
            if original_count >= target_samples:
                ...
                print(f"   [유지] 클래스 {label}: {original_count}개 (부족, 증강 없음)")
    ...
    def _print_distribution(self):
        ...
        출력 정보:
        1. 클래스별 샘플 수 및 비율
        2. 데이터셋별(UrbanSound8K, ESC-50, FSD50K) 샘플 수 및 비율
        """
        # 클래스별 분포
